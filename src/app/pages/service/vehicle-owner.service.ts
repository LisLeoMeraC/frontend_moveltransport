...
export class VehicleOwnerService {
  ...
  updateVehicleOwner(
    id: string, vehhicleOwnerData: VehicleOwnerData): Observable<ApiResponse<VehicleOwnerResponse>> {
    this.loading.set(true);
    this.error.set(null);

    return this.http.put<ApiResponse<VehicleOwnerResponse>>(`${this.baseUrl}/vehicle-owner/${id}`, vehhicleOwnerData
    ).pipe(
      tap({
        next: (response) => {
          if (response.statusCode >= 200 && response.statusCode < 300) { }
